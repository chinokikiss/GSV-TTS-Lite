...
class Text2SemanticDecoder(nn.Module):
    ...
    def initialize_runtime(self, dtype, device, gpt_cache):
        ...
        with stream_context:
            ...
            max_bucket = Bucket()
            max_bucket.k_cache = torch.empty((self.num_layers, max_btz, self.num_head, max_seqlen, int(self.model_dim / self.num_head)), dtype=dtype, device=device)
            max_bucket.v_cache = torch.empty((self.num_layers, max_btz, self.num_head, max_seqlen, int(self.model_dim / self.num_head)), dtype=dtype, device=device)
            max_bucket.decode_attn_mask = torch.zeros((max_btz, self.num_head, 1, max_seqlen), dtype=torch.bool, device=device)
            ...
            for batch_size in self.cuda_graph_buckets:
                ...
                for i in range(-1, -len(self.cuda_graph_buckets[batch_size])-1, -1):
                    ...
                    bucket.decode_attn_mask = max_bucket.decode_attn_mask[:batch_size, :, :, :max_kv_cache]
